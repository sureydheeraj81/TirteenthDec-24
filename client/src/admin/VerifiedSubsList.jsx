...
const VerifiedSubsList = () => {
  ...
  const subscriptionData = async () => {
    try {
      ...
      if (response.success) {
        setData(
          response.data.data.filter((elem) => elem.status === "Verified")
        );
      } else {
  ...
  const exportToExcel = () => {
    ...
    XLSX.writeFile(wb, "Verified.xlsx");
  };
        ...
        <div>
          <div className="box_header">
            <div style={{ padding: "5px 0px" }}>
              ...
              Verified User List {data.length}
            </div>
          </div>
          <div>
                      ...
                      data.map((elem, idx) => {
                        return (
                          <tr key={idx}>
                            ...
                            <td>{elem.mobile}</td>
                            <td>
                              <div>{elem.payment_verified_by}</div>
                              <div>{elem.payment_verification_date}</div>
                            </td>
                            <td>
                              <button
                                className="btn"

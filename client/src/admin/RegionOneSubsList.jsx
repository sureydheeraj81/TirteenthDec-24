...
const RegionOneSubsList = () => {
  ...
  const subscriptionData = async () => {
    try {
      ...
      if (response.success) {
        setData(
          response.data.data.filter(
            (elem) =>
              elem.status === "Approved" && elem.region_name === "region-1"
          )
        );
      } else {
      ...
      toast.error("Error in getting accepted subscription list");
    }
  };
  ...
  const exportToExcel = () => {
    ...
    XLSX.writeFile(wb, "RegisonOneSubscriptionList.xlsx");
  };
